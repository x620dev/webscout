...
| `--headful` | | Показать браузер (нужно для ручного решения капчи) | false |
...
При обнаружении капчи скрапер уведомляет и ставит на паузу до 120 секунд. По умолчанию браузер работает в фоне (`headless: true`) — решить капчу в нём невозможно.

**Быстрый способ** — добавить `--headful` к команде:

```bash
webscout org scrape yandex-maps -q "маникюр" --city "Уфа" --headful
```

Откроется видимое окно браузера. Решите капчу вручную — скрапер продолжит автоматически.

**Постоянно** — установить в `config.yaml`:
