...
- Если `-o` указан — принимается имя файла; директория (`data/json/`, `data/csv/`, `data/ai/`) подставляется автоматически по формату
...
Яндекс.Карты периодически показывают капчу при интенсивном скрапинге. По умолчанию браузер запущен в фоне (`headless: true`) — решить капчу в нём невозможно.

При появлении капчи запустите с флагом `--headful`: откроется видимое окно браузера, решите капчу вручную, скрапинг продолжится автоматически.

```bash
webscout org scrape yandex-maps --query "маникюр" --city "Уфа" --headful
```

Чтобы браузер всегда был видимым, установите в `config.yaml`:
```yaml
browser:
  headless: false
```
